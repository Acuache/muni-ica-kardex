...
`muni-ica-kardex` — aplicación de **kardex / almacén** para la Municipalidad de Ica. El scaffold (`create-next-app`, App Router) ya está en pie y la lógica de dominio se construye spec por spec (ver «Roadmap / especificaciones» abajo). **Implementado:** autenticación Supabase (Spec 01), roles/perfiles + shells por rol (Spec 02), el **catálogo** — categorías y productos (CRUD admin, imagen opcional en Storage, filtro/orden) (Spec 03), **áreas y gestión de usuarios** (Spec 04), **movimientos de kardex y stock** — entradas/salidas atómicas vía función Postgres transaccional con bloqueo de fila, soft-delete de productos (Spec 05), **vale de salida en PDF** con folio correlativo, servido por Route Handler con guard propio (Spec 06), y **movimientos multiproducto agrupados por categoría con lotes** (Spec 06.1): un registro lleva varios productos de cualquier categoría en un **lote** con código correlativo `L-000001`, se agrupan por categoría en el formulario y en un **vale consolidado por lote**, y la vista de Movimientos lista los lotes con filas expandibles. Falta el **dashboard** (Spec 07) y la **vista de usuario / mi historial** (Spec 08).
...
Stack: **Next.js 16.2.10**, **React 19.2.4**, **TypeScript 5** (strict), **Tailwind CSS v4** + shadcn (`base-luma`/`mist`). Capa de datos y auth: **Supabase** (Auth + Postgres + RLS + **Storage**) vía `@supabase/ssr` — cliente SSR y refresco de sesión ya cableados (Spec 01). Roles en tabla `profiles` con columna `role` (`superadmin`/`admin`/`usuario`), ya implementada (Spec 02). **Formularios (desde Spec 03):** `react-hook-form` + `zod` + `@hookform/resolvers`; las **imágenes de producto** se optimizan en el navegador con `browser-image-compression` (WebP) antes de subirlas a un bucket público de Supabase Storage. Los **vales de salida en PDF** se generan en el servidor con `@react-pdf/renderer` (Spec 06). Tests con **Vitest + React Testing Library** (ver «Tests»).
